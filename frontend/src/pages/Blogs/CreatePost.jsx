...
export default function CreateBlog() {
  ...
  return (
    <>
      ...
      {/* Main Wrapper: Applied slate-100 for light mode depth and custom hex for dark mode */}
        ...
        <div className="max-w-5xl mx-auto px-6 py-10 flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2.5">
              {/* Accent indicator changed to brand orange */}
          ...
          {/* Title Input */}
          <div className="bg-white dark:bg-[#12141c] border border-slate-300 dark:border-slate-800/70 rounded-md p-6 shadow-sm dark:shadow-none transition-colors">
            ...
            <input
              /* Input background is recessed (#0a0a0f) against the card (#12141c) in dark mode */
              className="w-full px-4 py-3 bg-white dark:bg-[#0a0a0f] border border-slate-300 dark:border-slate-800/80 rounded-md font-sans text-lg font-bold text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:border-orange-500 dark:focus:border-orange-500 transition-colors shadow-sm shadow-slate-200/50 dark:shadow-none"
              placeholder="Enter an engaging title..."
              value={title}
              onChange={e => setTitle(e.target.value)}
            />
            <div className="mt-2 font-mono text-[10px] font-semibold tracking-wider uppercase text-right">
              <span className={title.length > 0 ? 'text-orange-600 dark:text-orange-500' : 'text-slate-400 dark:text-slate-600'}>
            ...
            <div className="w-full sm:w-auto text-center sm:text-right">
              {!title.trim() || !content.trim() ? (
                ...
                <span className="inline-flex items-center justify-center sm:justify-end gap-2 text-green-600 dark:text-emerald-400 bg-green-50 dark:bg-emerald-500/10 px-3 py-1.5 rounded-[3px] border border-green-200 dark:border-emerald-500/20 font-mono text-[10px] font-bold uppercase tracking-widest">
                  <span className="w-1.5 h-1.5 rounded-full bg-green-500 dark:bg-emerald-400 shadow-[0_0_6px_#22c55e] dark:shadow-[0_0_6px_#34d399]"></span>
                  Ready to publish
                </span>
              )}
            </div>
