      ...
      <div className="flex flex-col gap-2">
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="font-sans text-[13px] font-semibold text-slate-800 dark:text-slate-200 group-hover:text-orange-500 transition-colors leading-tight">Pro Access</span>
            <span className="font-sans text-[9px] text-slate-500 mt-1">Upgrade</span>
          </div>
        </button>
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="font-sans text-[13px] font-semibold text-slate-800 dark:text-slate-200 group-hover:text-blue-500 transition-colors leading-tight">Leaderboards</span>
            <span className="font-sans text-[9px] text-slate-500 mt-1">Global Ranks</span>
          </div>
        </button>
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="font-sans text-[13px] font-semibold text-slate-800 dark:text-slate-200 group-hover:text-emerald-500 transition-colors leading-tight">My Lists</span>
            <span className="font-sans text-[9px] text-slate-500 mt-1">Bookmarks</span>
          </div>
        </button>
      </div>
      ...
      <div className="flex flex-col gap-4 border-t border-slate-200 dark:border-slate-800 pt-4">
        <div className="flex items-center justify-between">
          ...
          {(selectedDifficulties.length > 0 || selectedTags.length > 0) && (
            <button
              onClick={() => { setSelectedTags([]); setSelectedDifficulties([]); setPage(1); }}
        ...
        {/* Difficulty */}
        <div className="flex flex-col gap-2">
          <span className="font-sans text-[12px] font-semibold text-slate-700 dark:text-slate-300">Difficulty</span>
          <div className="flex flex-wrap gap-2">
            {difficultyOptions.map((d) => {
              ...
              return (
                <button
                  key={d.id}
                  onClick={() => setSelectedDifficulties((prev) => active ? prev.filter((x) => x !== d.id) : [...prev, d.id])}
        ...
        {/* Topics */}
        <div className="flex flex-col gap-2">
          <span className="font-sans text-[12px] font-semibold text-slate-700 dark:text-slate-300">Topics</span>
        ...
        <main className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
            <div className="flex items-center gap-3">
              ...
              {!loading && (
                <span className="font-sans text-[10px] font-semibold text-slate-500 dark:text-slate-400 bg-slate-200 dark:bg-slate-800 px-2 py-0.5 rounded-[3px] border border-slate-300 dark:border-slate-700">
                  {total} Problems
                </span>
              )}
            </div>
          ...
          <div className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md shadow-sm overflow-hidden flex flex-col flex-1">
              ...
              <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
                    <th className="px-5 py-3 font-mono text-[10px] font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase w-16 text-center">ID</th>
                    <th className="px-5 py-3 font-mono text-[10px] font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase">Title</th>
                    <th className="px-5 py-3 font-mono text-[10px] font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase text-center w-32">Acceptance</th>
                    <th className="px-5 py-3 font-mono text-[10px] font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase text-center w-32">Difficulty</th>
                    <th className="px-5 py-3 font-mono text-[10px] font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase text-center w-20">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                  {loading ? (
                    Array.from({ length: limit }).map((_, idx) => (
                      <tr key={idx} className="animate-pulse bg-white dark:bg-slate-950">
                        <td className="px-5 py-4"><div className="h-3 bg-slate-100 dark:bg-slate-800 rounded-[2px] w-6 mx-auto"></div></td>
                        <td className="px-5 py-4"><div className="h-4 bg-slate-100 dark:bg-slate-800 rounded-[2px] w-48 mb-1.5"></div></td>
                        <td className="px-5 py-4"><div className="h-3 bg-slate-100 dark:bg-slate-800 rounded-[2px] w-12 mx-auto"></div></td>
                        <td className="px-5 py-4"><div className="h-4 bg-slate-100 dark:bg-slate-800 rounded-[2px] w-14 mx-auto"></div></td>
                        <td className="px-5 py-4"><div className="h-6 bg-slate-100 dark:bg-slate-800 rounded-[3px] w-6 mx-auto"></div></td>
                      </tr>
                    ))
                  ) : problems.length === 0 ? (
                    <tr>
                    ...
                    problems.map((problem) => (
                      ...
                      >
                          ...
                          <div className="flex flex-col items-center gap-1 w-full max-w-[80px] mx-auto">
                            ...
                            {problem.acceptance_rate !== null && (
                              <div className="w-full h-1 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                                <div 
                                  className="h-full bg-slate-400 dark:bg-slate-500 group-hover:bg-orange-500 transition-colors"
                        ...
                        <td className="px-5 py-4 text-center">
                          <span className={`font-sans text-[11px] font-semibold tracking-wide capitalize ${getDifficultyColor(problem.difficulty)}`}>
            ...
            <div className="border-t border-slate-200 dark:border-slate-800 px-5 py-3 bg-slate-50 dark:bg-slate-900/50 flex items-center justify-between mt-auto">
              ...
              <div className="hidden sm:flex gap-1.5 items-center font-mono text-[11px] font-bold">
                {page > 2 && (
                  <>
                    <button onClick={() => setPage(1)} className="w-7 h-7 flex items-center justify-center cursor-pointer bg-transparent hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-[3px] transition-colors border border-transparent hover:border-slate-300 dark:hover:border-slate-700">1</button>
                    {page > 3 && <span className="w-7 h-7 flex items-center justify-center text-slate-400">...</span>}
                  </>
                )}

                {page > 1 && (
                  <button onClick={() => setPage(page - 1)} className="w-7 h-7 flex items-center justify-center cursor-pointer bg-transparent hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-[3px] transition-colors border border-transparent hover:border-slate-300 dark:hover:border-slate-700">
                    {page - 1}
                  </button>
                )}
                ...
                {page < totalPages - 1 && (
                  <>
                    ...
                    <button onClick={() => setPage(totalPages)} className="w-7 h-7 flex items-center justify-center cursor-pointer bg-transparent hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-[3px] transition-colors border border-transparent hover:border-slate-300 dark:hover:border-slate-700">
                      {totalPages}
                    </button>
                  </>
                )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-950 w-full max-w-md rounded-md shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden">
            ...
            <div className="px-5 py-3 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900">
              ...
              <button 
                onClick={() => { setShowAddToList(false); setActiveProblem(null); }}
            ...
            <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950">
              <p className="font-sans text-[15px] font-bold text-slate-900 dark:text-white">
                <span className="font-mono text-orange-500 mr-2">#{activeProblem?.id}</span> 
                {activeProblem?.title}
              </p>
            </div>
            ...
            <div className="p-5 max-h-[50vh] overflow-y-auto bg-slate-50 dark:bg-[#0d1117] custom-scrollbar flex flex-col gap-2">
              {listsLoading ? (
                <div className="py-10 flex justify-center items-center">
                  <span className="font-mono text-[10px] text-slate-500 uppercase tracking-widest animate-pulse">FETCHING...</span>
                </div>
              ) : userLists.length === 0 ? (
                <div className="py-8 text-center flex flex-col gap-2">
                  <span className="font-mono text-[11px] font-bold text-slate-500 uppercase tracking-widest">[ NO LISTS FOUND ]</span>
                  <span className="font-sans text-[13px] text-slate-400">Initialize a new list from your workspace.</span>
                </div>
              ) : (
                userLists.map((list) => (
                  <button
