  ...
  if (loading) {
    return (
      <div className="w-full h-[calc(100vh-4rem)] flex flex-col items-center justify-center bg-[#f8f9fa] dark:bg-[#0a0c10] transition-colors duration-300">
        ...
        <p className="text-slate-500 dark:text-slate-400 font-medium">Loading problem workspace...</p>
      </div>
    );
  }
    ...
    return (
      <div className="w-full h-[calc(100vh-4rem)] flex items-center justify-center bg-[#f8f9fa] dark:bg-[#0a0c10] transition-colors">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 text-center max-w-md">
          ...
          <p className="text-slate-900 dark:text-white font-semibold text-lg">{error}</p>
        </div>
      </div>
    );
  }
                  ...
                  <div key={i} className="border mt-4 border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-white dark:bg-slate-900 transition-colors">
                    ...
                    <div className="flex flex-col md:flex-row">
                      <div className="flex-1 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-700 p-4 bg-slate-50/50 dark:bg-slate-900/50">
                        <div className="text-[11px] uppercase tracking-wider font-bold text-slate-500 dark:text-slate-400 mb-2">Input</div>
                        <pre className="text-[13px] font-mono text-slate-800 dark:text-slate-200 whitespace-pre-wrap">{s.input}</pre>
                      </div>
                      <div className="flex-1 p-4 bg-white dark:bg-slate-900">
                        <div className="text-[11px] uppercase tracking-wider font-bold text-slate-500 dark:text-slate-400 mb-2">Output</div>
                        <pre className="text-[13px] font-mono text-slate-800 dark:text-slate-200 whitespace-pre-wrap">{s.output}</pre>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="space-y-4">
              ...
              {runResults.length === 0 ? (
                <div className="text-center py-16 flex flex-col items-center">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 font-medium">Run your code to evaluate sample test cases</p>
                </div>
              ) : (
                  ...
                  {runResults.map((r) => (
                    <div key={r.sample} className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-white dark:bg-slate-900 transition-colors">
                      <div className="bg-slate-50 dark:bg-slate-800/80 px-4 py-3 border-b border-slate-200 dark:border-slate-700">
                        <span className="font-bold text-slate-800 dark:text-slate-200">Test Case {r.sample}</span>
                      </div>
                      <div className="p-4 space-y-4">
                        <div>
                          <p className="text-[12px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Your Output</p>
                        ...
                        <div>
                          <p className="text-[12px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Expected Output</p>
                          <pre className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-3 rounded-lg text-[13px] font-mono text-slate-800 dark:text-slate-200">
                            {r.expected}
                          </pre>
                        </div>
            ...
            <div className="space-y-4">
              {!lastResult ? (
                <div className="text-center py-16 flex flex-col items-center">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 font-medium">No active submission found.</p>
                </div>
              ) : (
                <>
                  ...
                  <div className="border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-900 overflow-hidden transition-colors">
                    <div className="bg-slate-50 dark:bg-slate-800/80 px-4 py-3 border-b border-slate-200 dark:border-slate-700">
                      <span className="font-bold text-slate-900 dark:text-white">Test Cases Breakdown</span>
                    </div>
                    <ul className="divide-y divide-slate-200 dark:divide-slate-800">
                      {lastResult.samples.map((s) => (
                        <li key={s.index} className="px-4 py-3 flex items-center justify-between">
                          <span className="text-[13px] font-medium text-slate-700 dark:text-slate-300">Sample #{s.index}</span>
            ...
            <div className="space-y-3">
              {submissions.length === 0 ? (
                <div className="text-center py-16 flex flex-col items-center">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 font-medium">No past submissions</p>
                </div>
              ) : (
      ...
      <section className="w-full min-h-[calc(100dvh-4rem)] shrink-0 md:shrink md:min-h-0 md:h-full md:flex-1 flex flex-col bg-white dark:bg-[#1e1e1e] overflow-hidden transition-colors border-t md:border-t-0 border-slate-200 dark:border-slate-800">
        ...
        <div className="h-14 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 shadow-sm z-10 flex-shrink-0 transition-colors">
          ...
          <div className="flex gap-3 items-center">
            {authLoading ? (
              <span className="text-sm font-medium text-slate-500 dark:text-slate-400">Checking auth...</span>
            ) : user ? (
              <>
                <button
                  onClick={handleRun}
        ...
        <div className="fixed inset-0 bg-slate-900/40 dark:bg-black/60 flex items-center justify-center z-[150] backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-4xl h-[85vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800 transition-colors">
            ...
            <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 transition-colors">
              <div className="flex items-center gap-4">
                <h2 className="text-[18px] font-bold text-slate-900 dark:text-white">Submission Details</h2>
                <span
            ...
            <div
              className={`flex-shrink-0 overflow-hidden transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] ${
                complexity && complexity.id === openSubmission.id ? "max-h-40 opacity-100 border-b border-blue-100 dark:border-blue-900/30" : "max-h-0 opacity-0"
              }`}
            >
              <div className="px-6 py-4 bg-blue-50 dark:bg-blue-500/10 text-sm flex flex-col gap-2 transition-colors">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-bold tracking-tight text-blue-700 dark:text-blue-400">AI Analysis Insight</span>
                </div>
                ...
                <div className="flex flex-wrap gap-x-8 gap-y-3 pl-7">
                  <p className="flex items-center gap-2">
                    ...
                    <span className="bg-white dark:bg-slate-800 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700 shadow-sm"><InlineMath math={complexity?.time} /></span>
                  </p>
                  <p className="flex items-center gap-2">
                    <span className="font-bold text-slate-700 dark:text-slate-300">Space:</span>
                    <span className="bg-white dark:bg-slate-800 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700 shadow-sm"><InlineMath math={complexity?.space} /></span>
                  </p>
                </div>
              </div>
            </div>
