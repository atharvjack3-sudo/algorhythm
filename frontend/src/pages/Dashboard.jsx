  ...
  return (
    <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} className="w-full h-full block font-mono">
      ...
      {CF_BANDS.map((band, i) => {
        ...
        return <rect key={i} x="0" y={topY} width={svgWidth} height={bottomY - topY} fill={band.color} opacity="0.08" className="dark:opacity-[0.12]" />;
      })}
      ...
      <path d={linePath} fill="none" stroke="#f59e0b" strokeWidth="2" filter="url(#glow)" />
        ...
        <div className="max-w-6xl mx-auto py-10 px-6 flex flex-col gap-8">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-md shadow-sm border border-slate-200 dark:border-slate-800 p-8 relative overflow-hidden transition-colors">
            <div className="flex flex-col md:flex-row items-center md:items-start gap-8 relative z-10">
              ...
              {/* Avatar */}
              <div className={`w-28 h-28 rounded-md flex items-center justify-center font-sans text-5xl font-bold text-slate-900 dark:text-white shadow-sm flex-shrink-0 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800`}>
              ...
              <div className="flex-1 w-full text-center md:text-left">
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div>
                    <div className="flex items-center justify-center md:justify-start gap-3 flex-wrap">
                      <h1 className={`font-sans text-3xl font-bold tracking-tight`} style={{ color: getContestColor(contestStats?.contest_rating || 1200) }}>
                        {user.username}
                      </h1>
                      {status === "Premium" && (
                  ...
                  <button disabled={true} className="font-mono disabled:cursor-not-allowed text-[11px] font-semibold tracking-[0.06em] rounded-[3px] bg-transparent text-slate-600 dark:text-slate-300 border border-slate-300 dark:border-slate-700 px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors uppercase">
                    Edit Profile
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                  <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md p-4 flex flex-col gap-1.5">
                    <p className="font-mono text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Global Rank</p>
                    <p className="font-mono text-xl font-bold text-slate-900 dark:text-white">#{rank}</p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md p-4 flex flex-col gap-1.5">
                    <p className="font-mono text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Solved</p>
                    <p className="font-mono text-xl font-bold text-slate-900 dark:text-white">
                      {statsLoading ? "..." : solvedCount}
                    </p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md p-4 flex flex-col gap-1.5">
                    <p className="font-mono text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Acceptance</p>
                    <p className="font-mono text-xl font-bold text-slate-900 dark:text-white">
                      {acceptanceRate}%
                    </p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md p-4 flex flex-col gap-1.5">
                    <p className="font-mono text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Last Streak</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-md shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col">
              ...
              <div className="p-5 pb-0">
                <div className="flex items-baseline gap-3">
                  ...
                  {ratingHistory.length > 0 && (
                    <span className={`font-mono text-sm font-bold tracking-wider ${ratingHistory[ratingHistory.length - 1].rating_change >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
              ...
              <div className="p-5 h-[260px] w-full mt-auto bg-white dark:bg-slate-900">
                {ratingLoading ? (
                  <div className="w-full h-full flex items-center justify-center font-mono text-[11px] text-slate-400 uppercase">Loading Graph...</div>
                ) : (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-md shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col">
            <div className="px-5 py-3.5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-950">
              <div className="flex items-center gap-2.5">
                <span className={`inline-block w-[3px] h-[14px] rounded-sm ${showBadges ? 'bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]' : 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]'}`} />
                <span className="font-mono text-[11px] font-semibold tracking-[0.12em] text-slate-500 dark:text-slate-400 uppercase">
            ...
            <div className="p-6 bg-slate-50 dark:bg-slate-950 flex flex-col items-center sm:items-start overflow-hidden">
              {showBadges ? (
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 min-h-[160px] w-full">
                  {badgesLoading ? (
                    <div className="font-mono text-[11px] text-slate-400 uppercase tracking-widest w-full text-center">Loading Badges...</div>
                  ) : badgeList?.length ? (
                    badgeList.map((badge) => (
                      <div
                        key={badge.badge_id}
                    ...
                    <div className="text-center w-full">
                      <p className="font-mono text-[11px] text-slate-500 dark:text-slate-400 tracking-[0.08em] uppercase">No badges earned yet.</p>
                      <p className="font-sans text-[13px] text-slate-400 dark:text-slate-500 mt-2">Keep solving problems to unlock them!</p>
                    </div>
                  )}
                </div>
              ) : (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-md shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
            ...
            <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
              {recentLoading ? (
                <div className="p-8 text-center font-mono text-[11px] text-slate-400 uppercase tracking-widest">Loading...</div>
              ) : recentAC.length === 0 ? (
                ...
                <table className="w-full border-collapse whitespace-nowrap text-left">
                  <thead className="bg-slate-50 dark:bg-slate-950/50 top-0 z-10 border-b border-slate-200 dark:border-slate-800">
                    <tr>
                      <th className="px-5 py-2.5 font-mono text-[10px] font-semibold tracking-[0.1em] text-slate-500 dark:text-slate-400 uppercase">Problem</th>
                      <th className="px-5 py-2.5 font-mono text-[10px] font-semibold tracking-[0.1em] text-slate-500 dark:text-slate-400 uppercase text-center">Lang</th>
                      <th className="px-5 py-2.5 font-mono text-[10px] font-semibold tracking-[0.1em] text-slate-500 dark:text-slate-400 uppercase text-center">Verdict</th>
                      <th className="px-5 py-2.5 font-mono text-[10px] font-semibold tracking-[0.1em] text-slate-500 dark:text-slate-400 uppercase text-right">Time</th>
                      <th className="px-5 py-2.5 font-mono text-[10px] font-semibold tracking-[0.1em] text-slate-500 dark:text-slate-400 uppercase text-right">Memory</th>
                      <th className="px-5 py-2.5 font-mono text-[10px] font-semibold tracking-[0.1em] text-slate-500 dark:text-slate-400 uppercase text-right">Date</th>
                    </tr>
                  </thead>
                    ...
                    {recentAC.map((s) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3 text-right font-mono text-[11px] text-slate-500 dark:text-slate-500">
                          {new Date(s.submitted_at).toLocaleString(undefined, { month: 'short', day: 'numeric'})}
                        </td>
                      </tr>
                    ))}
                  </tbody>
